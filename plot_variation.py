...
def main():
    ...
    for ax, g in zip(axes.flat, GROUPS):
        ...
        area_mean = x.groupby(d.utla).transform("mean")
        within = x - area_mean - x.groupby(d.year).transform("mean") + x.mean()
        between = x.groupby(d.utla).mean() - x.mean()
        rows.append(dict(drug=g, sd_within=within.std(), sd_between=between.std(),
                         within_p95_abs=np.percentile(np.abs(within), 95)))
    ...
    fig.suptitle("How much does prescribing vary within areas over time? Log rate deviations, 292 lower-tier authorities, 2014–2024",
